function IMG = GERT_DrawElement_Rectangle(params)

% IMG = GERT_DrawElement_Rectangle(params)
%
% DESCRIPTION:
%  This function will draw a single rectangle patch, according to the
%  parameters defined. Important parameters are the WIDTH and the HEIGHT
%  defining the rectangle, its OR, and the SIZE of the image patch, defined 
...
%
% EXAMPLE: 
%  % Create a blue rectangle against a red background
%  rect_params.width = 5;
...
%% Check the arguments
fnc_name = 'GERT_DrawElement_Rectangle';
...
if GERT_elerrcheck
    ...
    if any([width height] > sz*2)
       msg = 'Width and height should exceed the size of the image.';
       GERT_ShowError(fnc_name,msg,3);
    end
...
points(:,3) = [0 + (width/2), 0 + (height/2)];
points(:,4) = [0 + (width/2), 0 - (height/2)];
